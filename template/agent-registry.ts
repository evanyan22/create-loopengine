...
// makes agents pluggable. Defining a new agent means adding a file to
// agents/ that exports `config` and `createModelCall` — nothing here or
// in any adapter needs to change; discoverAgents scans that directory and
// keys each entry by AgentConfig.name (not the filename) at import time.
import { dirname, join } from 'node:path'
import { fileURLToPath } from 'node:url'
import { discoverAgents, type AgentModule } from 'loopengine'
