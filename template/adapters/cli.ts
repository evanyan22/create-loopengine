...
//
//   npm run cli -- --agent example-agent --json "what's the weather in Boston?"
//
// --json prints the same typed LoopEvent stream adapters/http.ts's SSE
// route sends as one JSON object per line on stdout, ending with a
// 'done'/'error' event — nothing human-readable mixed in, so it's
// pipeable straight into `jq` or another process. Without --json, stdout
// stays human-oriented (just the final answer text) and events go to
// stderr as a one-line summary each.
//
...
// (see loopengine's own createSessionStore), a local .sessions/ dir
// otherwise — either way, safe for concurrent invocations against the
// same --session.
import { randomUUID } from 'node:crypto'
import { createSessionStore, runAgent, type LoopEvent } from 'loopengine'
...
async function main() {
  ...
  try {
    ...
    const text = await sessions.withSession(`${agent}:${sessionId}`, async (history) => {
      ...
      // synthetic stopReasons alike, so a --json consumer always sees
      // exactly one terminal event regardless of how the turn actually
      // ended.
